...
\ctexset{chapter={format=\raggedright,nameformat=\Large\bfseries\color{ink},titleformat=\Huge\bfseries\color{ink},name={第,章},aftername=\par,number=\arabic{chapter}},section={format=\Large\bfseries\color{ink}},subsection={format=\large\bfseries\color{accent}}}
...
% \textbf{版}保留定义、主要结论、基础证明、全部原有例题的题意摘要及解题方向，可作课堂讲义。\textbf{教师版}在相同主线中增加证明拆解、例题完整推导、反例、追问、算法边界与课堂组织建议。两版均已经校订，并非原稿的逐字存档。
...
\begin{teach}[每个定理的四步讲法]
\begin{enumerate}
\item 先提出一个朴素算法或具体算例，明确真正昂贵的对象。
\item 说明不变量与适用条件，再给定理；要求学生主动说出“不满足条件会怎样”。
\item 证明至少解释关键一步：为什么没有漏解、为什么可以约分、为什么每个合数只处理一次。
\item 把数学变成接口：输入、输出、边界、复杂度、溢出风险、最小反例。
\end{enumerate}
教师版的“课堂追问”可先遮住答案；例题解析不建议原样念完，应在关键换元前暂停。
\end{teach}
